...
export async function POST(request: NextRequest, { params }: { params: { siteId: string } }) {
  try {
    ...
    const { url: bodyUrl } = await request.json().catch(() => ({} as Record<string, unknown>))
    ...
    const created: Array<{ url: string; strategy: Strategy; perfScore?: number; lcpMs?: number; inpMs?: number; cls?: number; ttfbMs?: number; date: Date }> = []
